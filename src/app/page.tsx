...
export default function Home() {
  ...
  async function refreshStudentData() {
    try {
      ...
      setPenalties(
        await request<Penalty[]>("/student/penalties", {
          headers: authHeaders,
        }),
      );
      ...
      <main className="min-h-screen bg-[#f5f7f8] text-slate-950">
        <section className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-5">
          <div className="mb-8">
            ...
            <h1 className="mt-2 text-3xl font-semibold">Iniciar sesion22</h1>
    ...
    <main className="min-h-screen bg-[#f5f7f8] text-slate-950">
      ...
      <section className="mx-auto max-w-6xl px-5 py-6">
        ...
        <nav className="mb-5 flex flex-wrap gap-2">
          ...
          {isAdmin &&
            tabButton("admin", "Crear usuarios", activeTab, setActiveTab)}
        </nav>
          ...
          <section className="grid gap-5 lg:grid-cols-[360px_1fr]">
            ...
            >
              ...
              <Field
                ...
                onChange={(title) => setBookForm({ ...bookForm, title })}
              />
              <Field
                label="Genero"
                value={bookForm.genre}
                onChange={(genre) => setBookForm({ ...bookForm, genre })}
              />
              <Field
                label="Autor"
                value={bookForm.author}
                onChange={(author) => setBookForm({ ...bookForm, author })}
              />
                ...
                {request.status === "pending" && (
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => respondRequest(request.id, "approved")}
                    >
                      Aprobar
                    </button>
                    <button
              ...
              >
                <div>
                  <p className="font-medium">
                    {penalty.penaltyType?.name ?? "Penalizacion"}
                  </p>
                  <p className="text-sm text-slate-600">{penalty.reason}</p>
                  <p className="mt-1 text-sm">
                    {penalty.paidAt ? "Pagada" : "Pendiente"}
                  </p>
                </div>
                {!penalty.paidAt && (
                  <button
        ...
        {activeTab === "admin" && isAdmin && (
          <form
            onSubmit={createUser}
            className="max-w-md rounded-lg border bg-white p-4"
          >
            ...
            <Field
              ...
              onChange={(nextPassword) =>
                setUserForm({ ...userForm, password: nextPassword })
              }
            />
            <button className="mt-4 w-full rounded-md bg-slate-950 px-4 py-2 text-white">
              Crear
